...
FORBIDDEN_PASSWORDS = {
    ...
    "1234567890", "passw0rd", "qwerty123", "admin@123",
}
...
async def record_login_failure(username: str, db_session=None):
    ...
    if db_session is not None:
        ...
        if row:
            ...
            if row.login_fail_count >= MAX_LOGIN_FAILS:
...
class SessionManager:
    """基于内存的会话管理器，支持会话超时、并发会话数限制和定时清理。"""
...
class BruteForceProtection:
    """IP级暴力破解防护类，支持指数退避锁定策略。"""

    EXPONENTIAL_BACKOFF: Dict[int, int] = {
        1: 1,
        2: 5,
        3: 15,
    }
    ...
    def __init__(self) -> None:
        """初始化BruteForceProtection，创建IP失败记录存储。"""
    ...
    def record_failed_attempt(self, ip_address: str, username: str = "") -> None:
        ...
        backoff_minutes = self.EXPONENTIAL_BACKOFF.get(record["count"])
    ...
    def cleanup_expired(self) -> int:
        ...
        for ip, record in self._ip_attempts.items():
            if (now - record["last_attempt"]).total_seconds() > self.ATTEMPT_TTL_SECONDS:
