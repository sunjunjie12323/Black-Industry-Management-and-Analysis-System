...
class IntelligencePipeline:
    def __init__(self, llm_service=None, db_session_factory=None, vector_store=None, knowledge_graph=None):
        self._llm = llm_service
    ...
    async def process_single(self, item: Dict, use_llm: bool = True, retry: int = 0) -> PipelineItemResult:
        ...
        try:
            ...
            if is_dup:
                ...
                result.risk_score = self._risk_scorer.compute_risk_score(
                    classification, result.entities, [], result.cheating_scenarios
                )
            ...
            for attempt in range(retry + 1):
                try:
                    classification = await self._classifier.classify(cleaned_content, use_llm=use_llm)
                    ...
                    entities = await self._extractor.extract(cleaned_content, use_llm=use_llm)
                    result.entities = entities
                    result.stage = PipelineStage.EXTRACT.value

                    risk_score = self._risk_scorer.compute_risk_score(
                        classification, entities, [], result.cheating_scenarios
                    )
