...
from app.db.tables import RawIntelligenceTable, ReportTable
...
async def auto_generate_reports(
    ...
    try:
        total_count_stmt = select(func.count()).select_from(RawIntelligenceTable)
        total_intel = (await db.execute(total_count_stmt)).scalar() or 0
        ...
        generated = []
        rows_to_insert: list[ReportTable] = []
        for i in range(0, len(intels), 10):
            ...
            try:
                ...
                if hasattr(report, "id") and report.id:
                    rows_to_insert.append(ReportTable(
                        id=getattr(report, "id", None) or "",
                        title=report.title,
        ...
        if rows_to_insert:
            try:
                ...
                logger.warning(f"Bulk insert reports failed: {exc}")
                await db.rollback()
        return {"generated": len(generated), "total_intel": total_intel, "reports": generated}
    except Exception as exc:
