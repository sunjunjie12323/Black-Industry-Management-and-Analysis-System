...
from app.utils.db_helpers import safe_json_loads_list
...
def _rule_row_to_dict(row) -> dict:
    conditions = safe_json_loads_list(row.conditions_json)
    return {
        "id": row.id,
        "name": row.name,
...
async def get_active_alerts(
    ...
    if engine is not None:
        try:
            ...
            logger.error(f"Alert engine get_active_alerts failed: {exc}")
            # 降级到数据库查询，不抛出异常
    ...
    return {"alerts": [], "total": 0, "message": "暂无活跃告警"}
...
async def get_alert_stats(request: Request, current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        return {"total": 0, "by_severity": {}, "by_status": {}, "data_source": "unavailable", "message": "告警统计数据不可用"}
...
async def get_alert_rules(request: Request, current_user: User = Depends(get_current_user)):
    engine = getattr(request.app.state, "alert_engine", None)
    if engine is None:
        return {"rules": [], "total": 0, "message": "告警引擎未初始化"}
    return {"rules": [r.to_dict() for r in engine.rules.values()], "total": len(engine.rules)}
...
async def test_trigger_alert(
    ...
    return {"triggered_alerts": len(alerts), "alerts": alerts}
...
async def create_alert_rule(
    ...
    await audit_log_async(
        ...
        status="success",
        details={"name": data.name, "severity": data.severity},
    )
...
async def batch_acknowledge_alerts(
    ...
    try:
        ...
        # Commit transaction after processing all alerts
        await db.commit()
    except Exception as exc:
        logger.error(f"Batch acknowledge transaction failed: {exc}")
        await db.rollback()
        raise AppException(detail="批量确认失败，已回滚所有更改", error_code="BATCH_OPERATION_FAILED", status_code=500)
