...
async def start_training(
    ...
    async def _monitor_and_update():
        ...
        for iteration in range(max_iterations):
            ...
            logger.error(f"Finetune monitor exceeded max iterations for task {task_id}")

    async def _safe_monitor_task():
        try:
            await _monitor_and_update()
        except Exception as exc:
            logger.error(f"Finetune training monitor task failed: {exc}")
...
async def evaluate_task(
    ...
    async def _safe_eval_task():
        try:
            ...
            logger.error(f"Finetune evaluation task failed: {exc}")
...
async def list_available_models(
    ...
    if settings.FINETUNE_MODELS_JSON:
        try:
            models = json.loads(settings.FINETUNE_MODELS_JSON)
...
async def list_finetune_methods(
    ...
    from app.config import settings
    if settings.FINETUNE_METHODS_JSON:
        try:
            methods = json.loads(settings.FINETUNE_METHODS_JSON)
