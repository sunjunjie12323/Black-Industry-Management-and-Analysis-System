...
async def list_terms(
    ...
    if engine is None:
        return {"items": [], "total": 0, "offset": offset, "limit": limit, "message": "暗语引擎未初始化"}
    try:
...
async def search_blacktalk(
    ...
    engine = get_blacktalk_engine(request)
    if engine is None:
        return {"query": q, "results": [], "total": 0, "message": "暗语引擎未初始化"}
    try:
...
async def get_stats(
    ...
    engine = get_blacktalk_engine(request)
    if engine is None:
        return {"total_terms": 0, "categories": {}, "sources": {}, "average_confidence": 0.0, "message": "暗语引擎未初始化"}
    try:
