...
class KnowledgeGraph:
    ...
    async def get_entities(self, entity_ids: Iterable[str]) -> Dict[str, Entity]:
        ids = set(entity_ids)
        return {eid: self._entities[eid] for eid in ids if eid in self._entities}

    async def get_relations(self, relation_ids: Iterable[str]) -> Dict[str, Relation]:
        ids = set(relation_ids)
        return {rid: self._relations[rid] for rid in ids if rid in self._relations}
