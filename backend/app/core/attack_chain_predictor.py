...
# Reference defaults - override via MITRE_TRANSITIONS_JSON / MITRE_ENTITY_TACTIC_JSON env vars
MITRE_TRANSITIONS = {
...
def _load_attack_chain_config():
    global MITRE_TRANSITIONS, ENTITY_TYPE_TO_TACTIC
    from app.config import settings
    if settings.MITRE_TRANSITIONS_JSON:
        try:
            MITRE_TRANSITIONS = json.loads(settings.MITRE_TRANSITIONS_JSON)
        except (json.JSONDecodeError, TypeError):
            pass
    if settings.MITRE_ENTITY_TACTIC_JSON:
        try:
            ENTITY_TYPE_TO_TACTIC = json.loads(settings.MITRE_ENTITY_TACTIC_JSON)
        except (json.JSONDecodeError, TypeError):
            pass


_load_attack_chain_config()
...
class LLMAttackReasoner:
    ...
    async def reason_about_prediction(
        ...
        try:
            response = await asyncio.wait_for(
                self._llm.chat(prompt, temperature=settings.LLM_TEMPERATURE_CREATIVE, max_tokens=settings.LLM_MAX_TOKENS_SHORT),
    ...
    async def assess_time_window(
        ...
        try:
            response = await asyncio.wait_for(
                self._llm.chat(prompt, temperature=settings.LLM_TEMPERATURE_CREATIVE, max_tokens=settings.LLM_MAX_TOKENS_BRIEF),
                timeout=self.LLM_CALL_TIMEOUT,
            )
            if isinstance(response, dict):
    ...
    async def generate_countermeasures(self, prediction: "PredictionResult") -> List[Dict]:
        ...
        try:
            response = await asyncio.wait_for(
                self._llm.generate_json(prompt, temperature=settings.LLM_TEMPERATURE_CREATIVE),
                timeout=self.LLM_CALL_TIMEOUT,
            )
