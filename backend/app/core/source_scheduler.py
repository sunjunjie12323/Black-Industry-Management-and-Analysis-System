...
class SourceScheduler:
    ...
    def _load_sources_from_config(self):
        ...
        if settings.SOURCE_SCHEDULER_SOURCES_JSON:
            try:
                custom_sources = json.loads(settings.SOURCE_SCHEDULER_SOURCES_JSON)
                for sdata in custom_sources:
                    priority_str = sdata.get("priority", "medium")
                    try:
                    ...
                    source = SourceConfig(
                        ...
                        name=sdata.get("name", ""),
                        source_type=sdata.get("source_type", "forum"),
                        priority=priority,
                        status=status,
                        interval_minutes=sdata.get("interval_minutes", 30),
                        max_results_per_cycle=sdata.get("max_results_per_cycle", 50),
