...
import {
  ...
  Tabs, Tooltip, Switch, Upload, Popconfirm,
} from 'antd';
import {
...
import {
  ...
  ScheduleOutlined, ExperimentOutlined,
...
const INTENT_MAP: Record<string, { color: string; label: string; bg: string }> = {
  benign: { color: '#22C55E', label: '正常', bg: '#22C55E14' },
...
const IntelPipeline: React.FC = () => {
  ...
  const fetchStats = useCallback(async () => {
    ...
    if (!mountedRef.current) { setStatsLoading(false); return; }
  ...
  const fetchSources = useCallback(async () => {
    ...
    try {
      ...
      if (mountedRef.current) setSources(Array.isArray(res?.data) ? res.data : []);
    } catch { /* ignore */ }
  ...
  const fetchScenarios = useCallback(async () => {
    try {
      ...
      if (mountedRef.current) setScenarios(Array.isArray(res?.data) ? res.data : []);
    } catch { /* ignore */ }
  }, []);
  ...
  const fetchSlang = useCallback(async (keyword?: string) => {
    ...
    try {
      ...
      if (mountedRef.current) setSlangData(Array.isArray(res?.data) ? res.data : []);
    } catch { /* ignore */ }
    if (mountedRef.current) setSlangLoading(false);
  }, []);

  const fetchHistory = useCallback(async () => {
    try {
      const res = await api.intelPipeline.getCollectionHistory(20);
      if (mountedRef.current) setHistoryData(Array.isArray(res?.data) ? res.data : []);
    } catch { /* ignore */ }
  }, []);
  ...
  const statItems = [
    { label: '已处理', value: totalAll, icon: <DatabaseOutlined style={{ fontSize: 18, color: '#3B82F6' }} />, color: '#3B82F6' },
    { label: '去重数', value: totalDuplicates, icon: <SafetyCertificateOutlined style={{ fontSize: 18, color: '#14B8A6' }} />, color: '#14B8A6' },
  ...
  const intentDistribution = Object.entries(
    ...
    value,
    color: INTENT_MAP[name]?.color || '#7C7F9A',
  }));

  const renderRiskScore = (score: number) => {
    const pct = Math.round(score * 100);
    const color = pct >= 70 ? '#DC2626' : pct >= 40 ? '#EA580C' : pct >= 20 ? '#EAB308' : '#22C55E';
    return (
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, marginTop: 16 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
          <div style={{ padding: 16, background: '#0B0D17', borderRadius: 10, border: '1px solid rgba(255,255,255,0.06)', textAlign: 'center' }}>
            <div style={{ fontSize: 11, color: '#7C7F9A', fontWeight: 600, letterSpacing: '0.05em', marginBottom: 8 }}>意图等级</div>
            <Tag style={{
              background: INTENT_MAP[intentLevel]?.bg || '#7C7F9A14',
              color: INTENT_MAP[intentLevel]?.color || '#7C7F9A',
              border: `1px solid ${INTENT_MAP[intentLevel]?.color || '#7C7F9A'}28`,
          ...
          <div style={{ padding: 16, background: '#0B0D17', borderRadius: 10, border: '1px solid rgba(255,255,255,0.06)', textAlign: 'center' }}>
            <div style={{ fontSize: 11, color: '#7C7F9A', fontWeight: 600, letterSpacing: '0.05em', marginBottom: 8 }}>风险评分</div>
            {renderRiskScore(riskScore)}
          </div>
          <div style={{ padding: 16, background: '#0B0D17', borderRadius: 10, border: '1px solid rgba(255,255,255,0.06)', textAlign: 'center' }}>
            <div style={{ fontSize: 11, color: '#7C7F9A', fontWeight: 600, letterSpacing: '0.05em', marginBottom: 8 }}>质量评分</div>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 20, fontWeight: 700, color: '#E8E9ED' }}>
        ...
        {r.cleaned_content ? (
          <div style={{ padding: 16, background: '#0B0D17', borderRadius: 10, border: '1px solid rgba(255,255,255,0.06)' }}>
            <div style={{ fontSize: 12, color: '#7C7F9A', fontWeight: 600, marginBottom: 8, letterSpacing: '0.05em' }}>清洗后内容</div>
        ...
        {entities.length > 0 && (
          <div style={{ padding: 16, background: '#0B0D17', borderRadius: 10, border: '1px solid rgba(255,255,255,0.06)' }}>
            <div style={{ fontSize: 12, color: '#3B82F6', fontWeight: 600, marginBottom: 10, letterSpacing: '0.05em' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {entities.slice(0, 30).map((e: unknown, i: number) => {
                ...
                const typeColor: Record<string, string> = {
                  url: '#3B82F6', account: '#7C3AED', crypto: '#EA580C', crypto_address: '#EA580C',
                  tool: '#DC2626', slang: '#EAB308', contact: '#14B8A6',
                  organization: '#3B82F6', ip: '#7C7F9A', email: '#14B8A6', phone: '#14B8A6',
                };
                const et = String(ent.type || ent.entity_type || 'unknown');
                return (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}>
                    <Tag style={{
                      background: `${typeColor[et] || '#7C7F9A'}14`,
                      color: typeColor[et] || '#7C7F9A',
                      border: `1px solid ${typeColor[et] || '#7C7F9A'}28`,
                    ...
                    {Boolean((ent.metadata as Record<string, unknown>)?.meaning || ent.meaning) && (
                      <span style={{ color: '#7C7F9A', fontSize: 12 }}>— {String((ent.metadata as Record<string, unknown>)?.meaning || ent.meaning)}</span>
                    )}
                  </div>
                );
              })}
              {entities.length > 30 && (
                <div style={{ color: '#7C7F9A', fontSize: 12, textAlign: 'center', paddingTop: 4 }}>
    ...
    return (
      <div style={{ marginTop: 16 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 16 }}>
          {[
            { label: '总输入', value: Number(r.total_input ?? 0), color: '#3B82F6' },
            { label: '已处理', value: Number(r.total_processed ?? 0), color: '#22C55E' },
            { label: '去重数', value: Number(r.total_duplicates ?? 0), color: '#14B8A6' },
            { label: '高危数', value: Number(r.total_high_risk ?? 0), color: '#DC2626' },
          ].map(item => (
            <div key={item.label} style={{ padding: 14, background: '#0B0D17', borderRadius: 10, border: '1px solid rgba(255,255,255,0.06)', textAlign: 'center' }}>
              <div style={{ fontSize: 11, color: '#7C7F9A', fontWeight: 600, marginBottom: 4 }}>{item.label}</div>
              ...
              columns={[
                {
                  ...
                  render: (v: unknown, record: Record<string, unknown>) => (
                    <div>
                      <span style={{ color: '#E8E9ED', fontSize: 12 }}>
                ...
                },
                {
                  ...
                  render: (v: unknown[]) => (
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: '#E8E9ED' }}>
                ...
                },
                {
                  ...
                  render: (v: unknown[]) => {
                    if (!Array.isArray(v) || v.length === 0) return <span style={{ color: '#7C7F9A' }}>—</span>;
                    return (
                      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                        ...
                        {v.length > 3 && <Tag style={{ fontSize: 10, color: '#7C7F9A' }}>+{v.length - 3}</Tag>}
                      </div>
                    );
                  },
                },
  ...
  const sourceColumns = [
    {
      ...
      render: (v: string) => <span style={{ fontWeight: 500, color: '#E8E9ED', fontSize: 13 }}>{v}</span>,
    },
    {
      title: '类型', dataIndex: 'source_type', width: 100,
      render: (v: string) => (
        <Tag style={{ background: '#3B82F614', color: '#3B82F6', border: '1px solid #3B82F628', borderRadius: 4, fontSize: 11 }}>
          {SOURCE_TYPE_MAP[v] || v}
        </Tag>
      ),
    },
    {
      ...
      render: (v: string) => {
        ...
        ) : <span style={{ color: '#7C7F9A' }}>{v}</span>;
      },
    },
    {
      ...
      render: (v: string) => {
        ...
        const cfg = statusCfg[v] || { color: '#7C7F9A', label: v };
        return (
          <Tag style={{ background: `${cfg.color}14`, color: cfg.color, border: `1px solid ${cfg.color}28`, borderRadius: 20, padding: '0 8px', fontSize: 11 }}>
            {cfg.label}
          </Tag>
    ...
    },
    {
      ...
      render: (v: number) => (
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: '#E8E9ED' }}>{v ?? 0}</span>
      ),
    },
    {
      title: '错误', dataIndex: 'consecutive_errors', width: 70, align: 'center' as const,
      render: (v: number) => (
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: v > 0 ? '#DC2626' : '#7C7F9A' }}>
          {v ?? 0}
        </span>
      ),
    },
        ...
        <Space size={4}>
          <Tooltip title="触发采集">
            <Button type="link" size="small" icon={<PlayCircleOutlined />}
              ...
              style={{ color: '#22C55E', fontSize: 12, padding: 0 }} />
          </Tooltip>
  ...
  const scenarioColumns = [
    ...
    },
    {
      ...
      render: (v: string) => <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: '#7C7F9A' }}>{v}</span>,
    },
    {
      title: '严重等级', dataIndex: 'severity', width: 100,
      render: (v: string) => {
        const cfg = THREAT_MAP[v] || { color: '#7C7F9A', label: v };
        return (
          <Tag style={{ background: `${cfg.color}14`, color: cfg.color, border: `1px solid ${cfg.color}28`, borderRadius: 20, padding: '0 8px', fontSize: 11 }}>
            {cfg.label}
          </Tag>
      ...
      render: (v: string[]) => (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
          {(v || []).slice(0, 8).map((kw, i) => (
            <Tag key={i} style={{ background: 'rgba(255,255,255,0.04)', color: '#7C7F9A', borderRadius: 4, fontSize: 11, padding: '0 6px' }}>
              {kw}
            </Tag>
          ))}
          {v && v.length > 8 && <Tag style={{ fontSize: 11, color: '#7C7F9A' }}>+{v.length - 8}</Tag>}
        </div>
      ),
    },
  ];
  ...
  const slangColumns = [
    ...
    },
    {
      ...
      render: (v: string) => <span style={{ color: '#E8E9ED', fontSize: 13 }}>{v}</span>,
    },
  ];
  ...
  return (
    <div style={{ minHeight: '100%', padding: 0, background: '#0B0D17', overflowX: 'hidden' }}>
      <div style={{ padding: '32px 32px 8px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h1 style={{ fontSize: 28, fontWeight: 600, fontFamily: 'var(--font-body)', color: '#E8E9ED', letterSpacing: '-0.025em', marginBottom: 4, lineHeight: 1.2 }}>
            情报流水线
          </h1>
          <p style={{ fontSize: 14, color: '#7C7F9A', margin: 0, fontFamily: 'var(--font-body)' }}>
            自动化情报采集、清洗、去重、分类、实体抽取与风险评分
          </p>
        </div>
        <Space size={8}>
          <Button icon={<ReloadOutlined />} onClick={() => { fetchStats(); fetchSources(); fetchHistory(); }} aria-label="刷新数据">
            刷新
          </Button>
        </Space>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, marginBottom: 16 }}>
          {statItems.map((item, idx) => (
            <div key={item.label} style={{
              padding: '20px 24px', background: 'rgba(20,22,37,0.80)', borderRadius: 12,
            ...
            >
              ...
              <div>
                <div style={{ fontSize: 12, color: '#7C7F9A', fontWeight: 500, marginBottom: 4, fontFamily: 'var(--font-body)' }}>
                  {item.label}
                </div>
                <CountUpNumber value={item.value} style={{ fontSize: 24, fontFamily: 'var(--font-mono)', fontWeight: 700, color: '#E8E9ED', letterSpacing: '-0.02em', lineHeight: 1 }} />
              </div>
            </div>
          ))}
        </div>
      ...
      {intentDistribution.length > 0 && (
        <div style={{ padding: '0 32px', marginBottom: 16 }}>
          <div style={{ padding: 24, background: 'rgba(20,22,37,0.80)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)' }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: '#E8E9ED', marginBottom: 16, fontFamily: 'var(--font-body)' }}>
              意图等级分布
            </div>
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={intentDistribution} layout="vertical" margin={{ top: 4, right: 20, left: 60, bottom: 4 }} barCategoryGap="20%" barGap={4}>
                ...
                <YAxis type="category" dataKey="name" tick={{ fill: '#7C7F9A', fontSize: 10, fontFamily: 'var(--font-mono)' }} axisLine={false} tickLine={false} width={56} />
        ...
        <Tabs activeKey={activeTab} onChange={setActiveTab} items={[
          {
            ...
            children: (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div style={{ padding: 24, background: 'rgba(20,22,37,0.80)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)' }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: '#E8E9ED', marginBottom: 16 }}>单条分析</div>
                    ...
                    <div style={{ display: 'flex', gap: 12 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'flex-end', paddingBottom: 2 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          ...
                          <span style={{ fontSize: 12, color: '#7C7F9A' }}>LLM增强</span>
                        </div>
                      </div>
                    </div>
                ...
                <div style={{ padding: 24, background: 'rgba(20,22,37,0.80)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)' }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: '#E8E9ED', marginBottom: 16 }}>批量分析</div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    <div style={{ padding: '8px 12px', background: '#3B82F610', borderRadius: 8, fontSize: 12, color: '#3B82F6' }}>
                      每行一条情报，系统将自动批量处理
                    </div>
                ...
                {analyzeResult && (
                  <div style={{ padding: 24, background: 'rgba(20,22,37,0.80)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)', gridColumn: '1 / -1' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                      <div style={{ fontSize: 14, fontWeight: 600, color: '#E8E9ED' }}>分析结果</div>
                ...
                {batchResult && (
                  <div style={{ padding: 24, background: 'rgba(20,22,37,0.80)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)', gridColumn: '1 / -1' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                      <div style={{ fontSize: 14, fontWeight: 600, color: '#E8E9ED' }}>批量分析结果</div>
            ...
            children: (
              <div style={{ padding: 24, background: 'rgba(20,22,37,0.80)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: '#E8E9ED' }}>采集源列表</div>
                  <Space size={8}>
                ...
                {historyData.length > 0 && (
                  <div style={{ marginTop: 24 }}>
                    <div style={{ fontSize: 14, fontWeight: 600, color: '#E8E9ED', marginBottom: 12 }}>采集历史</div>
                    <Table
                      ...
                      columns={[
                        ...
                        },
                        {
                          ...
                          render: (v: string) => (
                            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: '#7C7F9A' }}>
                        ...
                        },
                        {
                          ...
                          render: (v: number) => {
                            ...
                            const cfg = isError ? { color: '#EAB308', label: '部分' } : { color: '#22C55E', label: '成功' };
                        ...
                        },
                        {
                          ...
                          render: (v: number) => <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: Number(v ?? 0) > 0 ? '#DC2626' : '#7C7F9A' }}>{v ?? 0}</span>,
                        },
                      ]}
                      rowKey="cycle_id"
                      size="small"
          ...
          },
          {
            ...
            children: (
              <div style={{ padding: 24, background: 'rgba(20,22,37,0.80)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)' }}>
                <div style={{ fontSize: 14, fontWeight: 600, color: '#E8E9ED', marginBottom: 16 }}>
          ...
          },
          {
            ...
            children: (
              <div style={{ padding: 24, background: 'rgba(20,22,37,0.80)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: '#E8E9ED' }}>
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 8 }}>
          <div>
            <div style={{ fontSize: 12, color: '#7C7F9A', marginBottom: 4, fontWeight: 500 }}>源ID *</div>
          ...
          <div>
            <div style={{ fontSize: 12, color: '#7C7F9A', marginBottom: 4, fontWeight: 500 }}>名称 *</div>
            <Input placeholder="如: 微信公众号监控" value={String(sourceForm.name || '')}
              onChange={e => setSourceForm({ ...sourceForm, name: e.target.value })} />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div>
              <div style={{ fontSize: 12, color: '#7C7F9A', marginBottom: 4, fontWeight: 500 }}>类型 *</div>
            ...
            <div>
              <div style={{ fontSize: 12, color: '#7C7F9A', marginBottom: 4, fontWeight: 500 }}>优先级</div>
          ...
          <div>
            <div style={{ fontSize: 12, color: '#7C7F9A', marginBottom: 4, fontWeight: 500 }}>采集间隔（分钟，0=仅手动）</div>
          ...
          <div>
            <div style={{ fontSize: 12, color: '#7C7F9A', marginBottom: 4, fontWeight: 500 }}>关键词（逗号分隔）</div>
